...
function bestOdd(odds: Odd[]): number {
  return Math.max(...odds.map(o => o.price));
}
...
/** Simple implied probability from decimal odds */
function impliedProb(price: number): string {
  return ((1 / price) * 100).toFixed(1) + '%';
}

/** Green if price is the best, red if worst, default otherwise */
...
function StatCard({ label, value, sub, color = 'var(--acid)' }: {
  ...
  return (
    <div style={st.statCard}>
      <div style={{ fontFamily:'var(--font-mono)', fontSize:9, letterSpacing:3, color:'var(--text-muted)', marginBottom:6 }}>{label}</div>
      <div style={{ fontFamily:'var(--font-display)', fontSize:26, fontWeight:800, color, lineHeight:1 }}>{value}</div>
      {sub && <div style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'var(--text-muted)', marginTop:4, letterSpacing:1 }}>{sub}</div>}
    </div>
  );
}

function MarketTable({ market, matchOdds }: { market: Market; matchOdds: Odd[] }) {
  const { odds } = market;
    ...
    <div style={st.marketBlock}>
      ...
      <div style={st.oddsGrid}>
        {selections.map(sel => {
          ...
          const best    = Math.max(...selOdds.map(o => o.price));
          return (
                ...
                {selOdds.map((o, i) => (
                  <div key={i} style={st.oddPill}>
                    ...
                    <span style={{
                      ...
                      color: priceColor(o.price, selOdds),
                    }}>
                      {o.price.toFixed(2)}
                    </span>
      ...
      {expanded && (
        <div style={st.marketsWrap}>
          {match.markets.length === 0
            ...
            : match.markets.map((mkt, i) => (
                <MarketTable key={i} market={mkt} matchOdds={allOdds} />
              ))
          }
        </div>
      )}
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        setError('');
        // Auto-expand first 3 matches
        if (expanded.size === 0 && data.length > 0) {
          setExpanded(new Set(data.slice(0, 3).map((m: Match) => m.match_id)));
        }
      } else {
...
const st: Record<string, React.CSSProperties> = {
  ...
  ctrlBtn:    { background:'none', border:'1px solid var(--border-base)', color:'var(--text-muted)', fontFamily:'var(--font-mono)', fontSize:8, letterSpacing:2, padding:'5px 10px', cursor:'pointer', whiteSpace:'nowrap' },
  ...
  noMarkets:  { fontFamily:'var(--font-mono)', fontSize:10, color:'var(--text-muted)', letterSpacing:2, textAlign:'center', padding:16 },
  ...
  bookOdds:   { display:'flex', gap:8, flex:1, flexWrap:'wrap' },
