    ...
    <div style={m.overlay}>
      <div style={m.modal}>
        ...
        <div style={m.infoBar}>
          ...
          <span style={{ ...m.infoVal, fontFamily:'var(--font-mono)', fontSize:10 }}>
            {req.login_url}
          </span>
        </div>
        ...
        {req.otp_required && (
          <div style={{ ...m.infoBar, background:'rgba(255,179,0,0.1)', borderColor:'rgba(255,179,0,0.3)' }}>
            ...
            <span style={{ ...m.infoText, color:'var(--amber)' }}>
              OTP LIKELY REQUIRED — you will be prompted after login
            </span>
          </div>
        )}
        ...
        {req.phone_format && (
          <div style={m.infoBar}>
            ...
            <span style={{ ...m.infoVal, color:'var(--acid)' }}>
              {req.phone_format.example}
            </span>
          </div>
        )}
        ...
        <div style={m.warning}>
          ⚠ Credentials are used only for this research session and are not stored in plaintext.
        </div>
  ...
  return (
    <div style={m.overlay}>
      <div style={{ ...m.modal, maxWidth: 420 }}>
        ...
        <p style={{ fontFamily:'var(--font-mono)', fontSize:11, color:'var(--text-secondary)', marginBottom:20 }}>
          {req.msg}
        </p>
        <div style={m.field}>
        ...
        <div style={m.buttons}>
          ...
          <button
            ...
            style={{ ...m.submitBtn, opacity: otp.length < 4 ? 0.5 : 1 }}
          >SUBMIT OTP →</button>
        </div>
      </div>
    </div>
...
function ReportViewer({ sessionId, onClose }: { sessionId: number; onClose: () => void }) {
  ...
  const renderMd = (md: string) => {
    ...
    return lines.map((line, i) => {
      if (line.startsWith('# '))  return <h1 key={i} style={rp.h1}>{line.slice(2)}</h1>;
      if (line.startsWith('## ')) return <h2 key={i} style={rp.h2}>{line.slice(3)}</h2>;
      if (line.startsWith('### '))return <h3 key={i} style={rp.h3}>{line.slice(4)}</h3>;
      if (line.startsWith('```')) return <div key={i} style={rp.codeBlock}></div>;
      if (line.startsWith('| '))  return <div key={i} style={rp.tableRow}><code style={rp.code}>{line}</code></div>;
      if (line.startsWith('- '))  return <div key={i} style={rp.li}>• {line.slice(2)}</div>;
      if (line.trim() === '')     return <div key={i} style={{ height:8 }} />;
      return <p key={i} style={rp.p}>{line}</p>;
    });
  };
          ...
          <div style={{ display:'flex', gap:8 }}>
            <button
              onClick={() => {
                ...
                a.download = `research-session-${sessionId}.md`;
                a.click();
              }}
              style={rp.downloadBtn}
            >⬇ DOWNLOAD .MD</button>
            <button onClick={onClose} style={rp.closeBtn}>✕</button>
          </div>
        </div>
        <div style={rp.body}>
          {loading
            ? <div style={rp.loading}>Loading report...</div>
            : report
              ? <div style={rp.content}>{renderMd(report)}</div>
              : <div style={rp.loading}>Report not yet generated</div>
          }
        </div>
      </div>
    </div>
...
export default function ResearchPage() {
  ...
  // Reload sessions when research completes
  useEffect(() => {
    if (researchDone) { loadSessions(); }
  }, [researchDone, loadSessions]);
    ...
    <div style={s.root}>
      ...
      {researchDone && (
        <div style={s.completeBanner}>
          <span>✅ Research complete for <strong>{researchDone.domain}</strong></span>
          <div style={{ display:'flex', gap:8 }}>
      ...
      <div style={s.header}>
        <div>
          ...
          <p style={s.subtitle}>AUTONOMOUS BOOKMAKER INTELLIGENCE · MULTI-PHASE DEEP RESEARCH</p>
        </div>
      ...
      <div style={s.launchBar}>
        ...
        <div style={s.phaseHints}>
          {['1. Unauthenticated scan + JS analysis + error probing',
            '2. Login form detected → credential popup appears here',
            '3. Authenticated deep-dive + WebSocket capture + Aviator JS',
            '4. Full markdown research report generated'].map((h, i) => (
            <span key={i} style={s.phaseHint}>
              <span style={{ color:'var(--acid)' }}>0{i+1}</span> {h}
            </span>
          ))}
        </div>
      </div>
          ...
          {sessions.map(sess => (
            ...
            >
              <div style={s.sessionTop}>
                ...
                <span style={{ ...s.phaseBadge, color: PHASE_COLORS[sess.phase] || 'var(--text-muted)', borderColor: PHASE_COLORS[sess.phase] || 'var(--border-dim)' }}>
                  {sess.phase}
                </span>
              </div>
              <div style={s.sessionDomain}>
                BK #{sess.bookmaker_id}
              </div>
              ...
              {sess.phase === 'COMPLETE' && (
                <button
                  onClick={e => { e.stopPropagation(); setShowReport(sess.id); }}
                  style={s.reportBtn}
                >📄 VIEW REPORT</button>
              )}
            </div>
          ...
          <div style={s.tabs}>
            {(['logs','findings','endpoints'] as const).map(t => (
              <button key={t} onClick={() => setTab(t)} style={{ ...s.tab, ...(tab===t ? s.tabActive:{}) }}>
                ...
                {t === 'endpoints' && sessionDetail && (
                  <span style={s.tabCount}>{sessionDetail.research_endpoints?.length || 0}</span>
                )}
              </button>
            ))}
          </div>
              ...
              <div style={s.terminal}>
                <div style={s.termBar}>
                  <div style={s.termDots}>
                    {['#ff5f57','#ffbd2e','#28ca41'].map(c => <div key={c} style={{ ...s.termDot, background:c }} />)}
                  </div>
                ...
                <div style={s.termBody}>
                  {visibleLogs.length === 0
                    ? <div style={s.termEmpty}>
                        <div style={{ fontFamily:'var(--font-mono)', fontSize:24, color:'var(--acid)', animation:'pulse-dot 1s infinite' }}>█</div>
            ...
            <div style={s.findingsWrap}>
              {!selected
                ...
                : findings.length === 0
                  ...
                  : findings.map(f => (
                      <div key={f.id} style={{ ...s.findingCard, borderColor: f.severity==='CRITICAL' ? 'rgba(255,61,90,0.4)' : f.severity==='WARN' ? 'rgba(255,179,0,0.3)' : 'var(--border-dim)' }}>
                        ...
                        {f.code && (
                          <pre style={s.findingCode}>{f.code.slice(0,600)}</pre>
                        )}
                      </div>
                    ))
              }
                ...
                : (
                  <>
                    ...
                    {sessionDetail.production_endpoints?.length > 0 && (
                      <>
                        <div style={s.epSectionHeader}>✅ PRODUCTION ENDPOINTS (promoted to harvest)</div>
                        <div style={s.epTable}>
                          {sessionDetail.production_endpoints.map((ep: any, i: number) => (
                            <div key={i} style={s.epRow}>
                              <span style={{ ...s.epType, color: ep.type==='LIVE_ODDS'?'var(--green)':ep.type==='DEEP_MARKETS'?'var(--cyan)':'var(--acid)' }}>
                                {ep.type}
                              </span>
                      ...
                      {(sessionDetail.research_endpoints || []).map((ep: any, i: number) => (
                        <div key={i} style={{ ...s.epRow, opacity: ep.status >= 400 ? 0.5 : 1 }}>
                          ...
                          <div style={s.epBadges}>
                            ...
                            {ep.is_auth && <span style={{ color:'var(--green)', fontFamily:'var(--font-mono)', fontSize:8 }}>🔐AUTHED</span>}
...
const s: Record<string, React.CSSProperties> = {
  root: { display:'flex', flexDirection:'column', height:'calc(100vh - 92px)', animation:'fade-in 0.3s ease', overflow:'hidden' },
  ...
  launchRow: { display:'flex', gap:8, marginBottom:12 },
  ...
  sessionDomain: { fontFamily:'var(--font-mono)', fontSize:9, color:'var(--text-muted)', marginBottom:4 },
  sessionMeta: { display:'flex', gap:8, fontFamily:'var(--font-mono)', fontSize:8, color:'var(--text-muted)' },
  reportBtn: { marginTop:6, width:'100%', padding:'4px', background:'rgba(198,241,53,0.1)', border:'1px solid rgba(198,241,53,0.3)', color:'var(--acid)', fontFamily:'var(--font-mono)', fontSize:8, letterSpacing:1, cursor:'pointer' },
  ...
  logRow: { display:'flex', alignItems:'baseline', gap:8, padding:'3px 12px 3px 8px', fontFamily:'var(--font-mono)', fontSize:11, borderBottom:'1px solid rgba(255,255,255,0.02)', animation:'fade-in 0.15s ease' },
  ...
  findingCard: { border:'1px solid', padding:'12px', marginBottom:8, animation:'slide-up 0.2s ease' },
  ...
  completeBanner: { display:'flex', justifyContent:'space-between', alignItems:'center', background:'rgba(198,241,53,0.1)', border:'1px solid rgba(198,241,53,0.3)', padding:'10px 16px', marginBottom:12, flexShrink:0, animation:'slide-up 0.3s ease' },
...
const m: Record<string, React.CSSProperties> = {
  ...
  modal: { background:'var(--bg-surface)', border:'1px solid var(--border-bright)', width:'100%', maxWidth:520, padding:32, boxShadow:'0 32px 80px rgba(0,0,0,0.8)', animation:'slide-up 0.25s ease' },
...
const rp: Record<string, React.CSSProperties> = {
  overlay: { position:'fixed', inset:0, background:'rgba(7,8,10,0.9)', zIndex:999, display:'flex', flexDirection:'column' },
  ...
  loading: { textAlign:'center', padding:60, fontFamily:'var(--font-mono)', fontSize:11, color:'var(--text-muted)', letterSpacing:3 },
  ...
  codeBlock: { height:2, background:'var(--border-dim)', margin:'8px 0' },
